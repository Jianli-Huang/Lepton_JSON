...
static void test_parse_null() {
	...
	EXPECT_EQ_INT(LEPT_PARSE_OK, lept_parse(&v, "null"));   //解析成功
	EXPECT_EQ_INT(LEPT_NULL, lept_get_type(&v));            //解析出是LEPT_NULL类型
	lept_free(&v);
}
...
static void test_parse_true() {
	...
	lept_init(&v);
	lept_set_boolean(&v, 0);
	EXPECT_EQ_INT(LEPT_PARSE_OK, lept_parse(&v, "true"));   //解析成功
	EXPECT_EQ_INT(LEPT_TRUE, lept_get_type(&v));            //解析出是LEPT_TRUE类型
	lept_free(&v);
}
...
static void test_parse_false() {
	...
	EXPECT_EQ_INT(LEPT_PARSE_OK, lept_parse(&v, "false"));  //解析成功
	EXPECT_EQ_INT(LEPT_FALSE, lept_get_type(&v));           //解析出是LEPT_FALSE类型
	lept_free(&v);
}

//测试节点类型是数字的情况
//解析成功 -> 解析出是LEPT_NUMBER类型 -> 解析值跟输入的数字相同
...
//测试节点类型是字符串的情况
//解析成功 -> 解析出是LEPT_STRING类型 -> 解析值跟输入的字符串相同
...
{
    // "\"\"" 是一个const char* json，最外围的" "表示里面的内容是一段字符串，
    //实际的内容是\"\"，而也从这里开始解析，json[0] <-> \"
	TEST_STRING("", "\"\"");
...
//Unicode，如果只有高代理项而欠缺低代理项，或是低代理项不在合法码点范围
